...
, 'K', '%']) -> float:
    """Safely convert string values to numeric, removing specified characters"""
    try:
        clean_value = str(value_string)
...
def safe_dataframe_numeric_conversion(df: pd.DataFrame, column: str, remove_chars: List[str] = ['
...
, 'K', '%']) -> List[float]:
    """Safely convert DataFrame column to numeric values"""
    try:
        return [safe_string_to_numeric(str(val), remove_chars) for val in df[column]]
    except Exception:
        return [0.0] * len(df)
